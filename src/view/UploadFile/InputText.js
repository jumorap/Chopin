 ...
 * @returns 
 */
const InputText = ({label, options, optionLabel, setOption, errorState, setError}) => {
    ...
    return (
        <Autocomplete
            ...
            style={{ width: 300 }}                    
            renderInput={(params) => <TextField className = {classes.textField} {...params}   error = {errorState} helperText = {errorState?"Este campo no puede estar vacio":""} label = {label} variant="outlined"/>}
            className = {classes.container}
            onChange={(event, newValue) => {setOption(newValue);setError(false)}}
        />
    )
}
