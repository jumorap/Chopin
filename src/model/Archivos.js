...
class Archivos {
   ...
   */
  static deleteArchivos(id_materia, id_archivo, profesor, semestre, categorias){
    //delete record from materia filter
    Archivos._deleteMateriasTrabajos(id_materia, id_archivo)

    //delete record from filters
    Archivos._deleteMateriasFiltro(id_materia, id_archivo, "profesores", profesor)
    Archivos._deleteMateriasFiltro(id_materia, id_archivo, "semestres", semestre)
  ...
  static _deleteFile(id_materia,id_archivo){
    ...
    fileRef.delete()
    .then(()=>{console.log("File eliminado correctamente del storage")})
    .catch(()=>{console.log("Error al eliminar el file del storage")})
  }
   ...
   * @param  {String} valorFiltro filtro que se le va a añadir ej ("NombreProfesor(Korgi), Semestre(2021-1), Categoria(parcial 1)"
   */
  static _deleteMateriasFiltro(id_materia,id_archivo,nombreFiltro, valorFiltro){
    this._DBmateriasDisplay
      .doc(id_materia)
      .update({
        [`${nombreFiltro}.${valorFiltro}.${id_archivo}`]: firebase.firestore.FieldValue.delete(),
      })
      .then(() => {
  ...
  static _deleteMateriasTrabajos(id_materia, id_archivo){
    this._DBmateriasDisplay
          ...
          .then(()=>{console.log("archivo de materias borrado satisfactoriamente")})    
          .catch(()=>{console.log("no se pudo borrar el archivo")})
  }  
}
